...
+
E メールアラートの送信元の E メールアドレスを指定します。SnapManager 3.2 for SAPでは、Eメールアドレスのドメイン名を指定する際にハイフン（-）を使用できます。たとえば、送信者の電子メールアドレスを「+-sender-email07lbfmdatacenter@continental-corporation.com +」として指定できます。
...
* *`-authentication-username USERNAME-password PASSWORD_`*
+
